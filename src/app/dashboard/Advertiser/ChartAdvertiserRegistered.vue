...
export default {
    ...
    possuiItems() {
      ...
      if (dataAdvertiser.length) {
        return !!dataAdvertiser.find((advertiser) => {
          return (advertiser.anunciantes_comuns_cadastrados > 0 || advertiser.anunciantes_premiuns_cadastrados > 0);
        });
      }
      return false;
    }
  ...
  notifications: {
    showError: {
      title: 'Erro',
      message: 'Ocorreu um erro ao tentar pegar construir o gráfico de anunciantes registrados!!',
      type: 'error',
    },
  },
  ...
  methods: {
    ...
    mountOptionsVerticalChart(advertiser) {
      ...
      advertiser.map((advertiser, index) => {
        smarts.push(advertiser.anunciantes_comuns_cadastrados);
        premium.push(advertiser.anunciantes_premiuns_cadastrados);
    ...
    generateReportVerticalChart() {
      ...
      this
        .$router
        .push({ name: 'report-advertiser-registered', query: { data: date } });
    },
  },
  watch: {
    filters: {

<template lang="pug">
  #chartClientsRegistered
    chart(
      title="Quantidade de clientes cadastrados"
      :generateReportView="generateReportClientsRegistereds"
...
export default {
  ...
  created() {
    this.getCountClientsForChartRegistereds();
  },
  methods: {
    getCountClientsForChartRegistereds() {
      this.$loading.start();
      ...
      DashboardService
        .facade()
        .getCountClientsForChartRegistereds({ data })
        .then(({ data }) => {
    ...
    generateReportClientsRegistereds() {
      const { dateClientsRegistereds } = this.filters;
      const data = this.parseMonth(dateClientsRegistereds);
      this
        .$router
        .push({ name: 'report-clients-registered', query: { data } });
    },
  },
  watch: {
    filters: {

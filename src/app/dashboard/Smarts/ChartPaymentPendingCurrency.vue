<template lang="pug">
  #ChartPayment
    chart(
      title="Quantidade pendente de recebimento em reais por planos"
...
export default {
  name: 'chart-payment-pending-currency',
  ...
  methods: {
      ...
      this.planos.map((plano) => {
        ...
        payments.map((payment) => {
          ...
          if (typeof tipoPlano !== 'undefined') {
            eixoY[plano].push(tipoPlano.valor_em_reais);
          }
        });
    ...
    generateReportVerticalChart() {
      const { dateVerticalChart } = this.filters;
      const data = this.parseMonth(dateVerticalChart);
      this
        .$router
        .push({ name: 'report-receive-pending-currency-by-plan', query: { data }});
    },
  },

...
export default {
  ...
  created() {
    ...
    this.getClientsForReportRegistered();
  },
  methods: {
    verificaSeQueryEstaPreenchida() {
      ...
      this
        .$router
        .push({ name: 'report-clients-registered', query: { data, tipoUsuario } });
    },
    getClientsForReportRegistered() {
      this.$loading.start();
      const { paginaAtual, quantidadePorPagina } = this.pagination;
      const { ativo, data, tipoUsuario } = this.filters;
      const params = {
        inadimplente: false,
        data,

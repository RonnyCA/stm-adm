    ...
    report(
      ...
      template(slot="filters")
        table-filters(
          ...
          :statusSaque="solicitacaoSaque"
          @set-filter-plan="setFiltersPlan($event)"
          @set-filter-category="setFiltersCategory($event)"
          @set-filter-status-saque="setFilterssolicitacaoSaque($event)"
          )
      template(
        slot="table-row"
        slot-scope="items"
        ...
        td {{ items.item.smarts_acumulados }}
        td {{ items.item.solicitacao_saque }}
        td {{ items.item.atraso }}
</template>

<script>
import Report from '../Report.vue';
import TableFilters from './TableFilters.vue';
import Headers from './HeadersReportPaymentPendingByPlan';
...
export default {
  ...
  data: () => ({
    ...
    categorias: [],
    solicitacaoSaque: [
      { value: 'Pendente', text: 'Saque pendente'},
      { value: 'NaoSolicitado', text: 'Não solicitado'},
    ],
    filters: {
      plano: null,
      categoria: null,
      solicitacaoSaque: 'Pendente',
  ...
  created() {
    ...
    this.getAllPlan();
    this.getAllCategory();
    this.getSmartForReportPaymentPendingByPlan();
  },
  ...
  methods: {
    verifyIfQueryFilled() {
      ...
      if (!!query.solicitacaoSaque) {
        this.filters.solicitacaoSaque = query.solicitacaoSaque;
      }
      const queryString = {
      ...
      this
        .$router
        .push({ name: 'report-payment-pending-by-plan', query: queryString });
    },
    setPagination(event) {
    ...
    setFiltersStatusSaque(event) {
      this.filters.solicitacaoSaque = event;
      this.getSmartForReportPaymentPendingByPlan();
    },
    getAllPlan() {
      this.$loading.start();
    ...
    getSmartForReportPaymentPendingByPlan() {
      this.$loading.start();

      const { paginaAtual, quantidadePorPagina } = this.pagination;
      const { plano, categoria, data, solicitacaoSaque } = this.filters;
      const params = {
        data,
        planoId: plano,
